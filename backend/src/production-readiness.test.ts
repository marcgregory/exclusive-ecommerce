...
import { createSessionOptions, PostgreSqlSessionStore } from './session-store.js';
...
describe('session config', () => {
  it('uses PostgreSQL session storage only in production', () => {
    ...
    expect(createSessionOptions(productionConfig).store).toBeInstanceOf(PostgreSqlSessionStore);
...
describe('health, readiness, and error metadata', () => {
  ...
  it('reflects only configured CORS origins', async () => {
    process.env.WEB_ORIGINS = 'https://shop.example.com,https://preview.example.com';
    const { default: app } = await import('./index.js');
  ...
  it('accepts client error reports and logs a sanitized payload', async () => {
    ...
    expect(console.error).toHaveBeenCalledWith(expect.stringContaining('"event":"client.error"'));
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining('"requestId":"req-client-1"')
    );
    expect(console.error).not.toHaveBeenCalledWith(expect.stringContaining('not logged'));
  });
});
