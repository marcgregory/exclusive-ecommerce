...
export class LocalProductImageStorage implements ProductImageStorage {
  ...
  async saveProductImage(input: {
    ...
    const extension =
      imageTypeByContentType[validated.contentType as keyof typeof imageTypeByContentType];
...
export function uploadBufferToCloudinary(buffer: Buffer, options: UploadApiOptions) {
