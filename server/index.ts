...
app.patch("/api/admin/orders/:id", requireAdmin, adminWriteLimiter, asyncRoute(async (req, res) => {
...
app.post("/api/admin/products", requireAdmin, adminWriteLimiter, asyncRoute(async (req, res) => {
  const product = await createProduct(req.body);
  res.status(201).json({ product });
}));

app.patch("/api/admin/products/:id", requireAdmin, adminWriteLimiter, asyncRoute(async (req, res) => {
  const product = await updateProduct(req.params.id, req.body);
  if (!product) return res.status(404).json({ message: "Product not found" });
  res.json({ product });
}));

app.delete("/api/admin/products/:id", requireAdmin, adminWriteLimiter, asyncRoute(async (req, res) => {
...
// Categories
app.post("/api/admin/categories", requireAdmin, adminWriteLimiter, asyncRoute(async (req, res) => {
  const category = await createCategory(req.body);
  res.status(201).json({ category });
}));

app.patch("/api/admin/categories/:id", requireAdmin, adminWriteLimiter, asyncRoute(async (req, res) => {
  const category = await updateCategory(req.params.id, req.body);
  if (!category) return res.status(404).json({ message: "Category not found" });
  res.json({ category });
}));

app.delete("/api/admin/categories/:id", requireAdmin, adminWriteLimiter, asyncRoute(async (req, res) => {
...
// Coupons
app.post("/api/admin/coupons", requireAdmin, adminWriteLimiter, asyncRoute(async (req, res) => {
  const coupon = await createCoupon(req.body);
  res.status(201).json({ coupon });
}));

app.patch("/api/admin/coupons/:code", requireAdmin, adminWriteLimiter, asyncRoute(async (req, res) => {
  const coupon = await updateCoupon(req.params.code, req.body);
  if (!coupon) return res.status(404).json({ message: "Coupon not found" });
  res.json({ coupon });
}));

app.delete("/api/admin/coupons/:code", requireAdmin, adminWriteLimiter, asyncRoute(async (req, res) => {
...
// Contact messages
app.get("/api/admin/contact-messages", requireAdmin, asyncRoute(async (req, res) => {
...
app.patch("/api/admin/contact-messages/:id", requireAdmin, adminWriteLimiter, asyncRoute(async (req, res) => {
  const message = await updateContactMessageStatus(req.params.id, String(req.body.status || ""));
