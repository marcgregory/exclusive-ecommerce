...
function getSessionExpiration(sess: session.SessionData): Date {
  ...
  const maxAge =
    typeof cookie.originalMaxAge === 'number' ? cookie.originalMaxAge : DEFAULT_SESSION_TTL_MS;
...
export function createSessionOptions(config: RuntimeConfig): session.SessionOptions {
  return {
    name: 'exclusive.sid',
