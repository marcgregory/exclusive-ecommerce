  ...
  return (
    <main className="container page">
      ...
      <div className="contact-layout">
        ...
        <form className="contact-form" onSubmit={submit}><div className="three-col"><FormField name="name" label="Your Name" required register={register("name")} error={errors.name?.message} /><FormField name="email" label="Your Email" required register={register("email")} error={errors.email?.message} /><FormField name="phone" label="Your Phone" register={register("phone")} error={errors.phone?.message} /></div><FormField name="message" label="Your Message" textarea required register={register("message")} error={errors.message?.message} /><Button type="submit" disabled={isSubmitting}>{isSubmitting ? "Sending..." : "Send Message"}</Button>{status && <p className={`form-status ${statusIsError ? "form-status--error" : ""}`}>{status}</p>}</form>
      </div>
    </main>
  );
}
