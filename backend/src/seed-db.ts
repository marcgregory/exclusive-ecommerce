...
export async function seedDatabase(): Promise<void> {
  ...
  dotenvConfig({ path: resolve(backendDir, '.env.production') });
  ...
  if (process.env.NODE_ENV !== 'test') {
    if (
      ...
      throw new Error(
        'Cloudinary credentials are not available. Please check your environment variables.'
      );
    }
  ...
  await withTransaction(async (client) => {
      ...
      for (const color of colors) {
        for (const size of sizes) {
          await client.query(
            ...
            [
              `pv-${product.id}-${color || 'default'}-${size || 'default'}`,
              product.id,
              color,
              size,
              product.stockStatus === 'Out of Stock' ? 0 : 10,
            ]
          );
        }
      }
    ...
    for (const coupon of state.coupons) {
      await client.query(
        'INSERT INTO coupons (code, type, amount, active) VALUES ($1, $2, $3, $4)',
        [coupon.code, coupon.type, coupon.amount, coupon.active]
      );
    }
    ...
    for (const wishlist of state.wishlists) {
      ...
      await client.query('INSERT INTO wishlists (id, user_id) VALUES ($1, $2)', [
        wishlistId,
        wishlist.userId,
      ]);
      for (const productId of wishlist.productIds) {
        await client.query('INSERT INTO wishlist_items (wishlist_id, product_id) VALUES ($1, $2)', [
          wishlistId,
          productId,
        ]);
      }
